...
from read_write import read_matching_data_from_file_product, get_matching_filenames_across_saildrones, get_sd_file_from_match_filename, get_sat_file_from_match_filename, read_config, read_saildrone, read_swath
import pandas as pd
...
matching_fls = sorted(get_matching_filenames_across_saildrones(product=config["satellite_product"]))
...
for sd in sd_files_unique:
    tmp = round_coordinates(data=read_saildrone(filename=sd, masked_nan=True, to_pd=True))
...
for match_fl in matching_fls:
    tmp = round_coordinates(data=read_matching_data_from_file_product(filename=match_fl), vars=["sd_lon", "sd_lat", "st_lon", "st_lat", "dist"])
...
for nmatch in range(len(matching_fls)):
    ...
    tmp = match_saildrone_satellite_point(match_data=match_current, sd_data=sd_current, st_data=sat_current, sd_var=sd_var, st_var=st_var)
    tmp = tmp.dropna(axis=0, thresh=5)
    if len(tmp) > 0:
        combined.append(tmp)
        mean.append(tmp.groupby("sd_time", as_index=False).mean().reset_index(drop=True))
        nearest.append(tmp.loc[tmp.groupby("sd_time").dist.idxmin()].reset_index(drop=True))
...
filename = f"SD_" + f"{config['satellite_product']}_scatter_overlap_{config['saildrone_variable_name']}.png"
plot_scatterplot_overlap(combined_pts=combined, mean_pts=mean, nearest_pts=nearest, filename=filename, axmin=0, axmax=20)

filename = f"SD_" + f"{config['satellite_product']}_matching_points_locations_{config['saildrone_variable_name']}.png"
title = f"(SD - {config['satellite_product']}) matching point locations"
plot_matching_point_locations(match_data=match_data, sd_fls=sd_files, matching_fls=matching_fls, filename=filename, title=title)
